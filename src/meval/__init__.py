...
async def meval(
    ...
    if globs is None:
        try:
            ...
            raise RuntimeError("Unable to retrieve globals() from the caller frame")
    ...
    for glob in ["__name__", "__package__"]:
        context[global_args][glob] = globs.get(glob)
    ...
    args = []
    for a in list(map(lambda x: ast.arg(x, None), context.keys())):
    ...
    r = await locs["tmp"](**context)
    with suppress(TypeError):
        for i in range(len(r)):
            if inspect.isawaitable(r[i]):
                r[i] = await r[i]
        i = 0
        while i < len(r) - 1:
            if r[i] is None:
